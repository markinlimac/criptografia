...
def makeBitBlock(bloco, c_block, d_block, escolha): #transforma bloco de texto em um bloco de 64 bits
    ...
    if escolha == 1:
        ...
        crashBlock(bloco_bit, c_block, d_block, escolha)
    elif escolha == 2:
        inverseInitialPermutation(bloco_bit)
        crashBlock(bloco_bit, c_block, d_block, escolha)
...
def initialPermutation(bloco_bit):
    ...
    for i in range(8):
        for j in range(8):
            ...
            bloco_bit[i][j] = matriz_permutada[i][j]
...
def gravarDescriptografado(left_block, right_block):
    ...
    initialPermutation(bits_criptografados)
