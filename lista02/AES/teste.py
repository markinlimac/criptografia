...
class AESModeOfOperation(object):
    ...
    # converts a 16 character string into a number array de acordo com a tabela ascii
...
def encryptData(key, text, mode=AESModeOfOperation.modeOfOperation["CFB"]):
    ...
    iv = [ord(i) for i in os.urandom(32)] #cria um iv aleatorio
    moo = AESModeOfOperation()
    (mode, length, ciph) = moo.encrypt(text, mode, key, keysize, iv)
...
def generateRandomKey(keysize = 32):
...
if __name__ == "__main__":
    ...
    if escolha == 1:
        ...
        cipher = encryptData(key, text, mode)
        arq = open("cypher_text.txt", "w")
        arq.write(str(cipher))
        arq.close
        ...
        if existe == True: #Caso ja exista o arquivo "cypher_text.txt" o programa ira ler para descriptografar            
            ...
            decr = decryptData(key, text, mode)
